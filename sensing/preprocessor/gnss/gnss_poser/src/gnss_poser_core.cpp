  ...
  plane_zone_(9)
{
  ...
  private_nh_.getParam("origin_lat", origin_lat_);
  private_nh_.getParam("origin_lon", origin_lon_);
  private_nh_.getParam("origin_alt", origin_alt_);

  origin_status_ = lla2UTM(origin_lat_, origin_lon_, origin_alt_);
...
void GNSSPoser::callbackNavSatFix(const sensor_msgs::NavSatFix::ConstPtr & nav_sat_fix_msg_ptr)
{
  ...
  auto position = getPosition(gnss_stat);
  position.x = gnss_stat.x - origin_status_.x;
  position.y = gnss_stat.y - origin_status_.y;
  position.z = gnss_stat.z - origin_status_.z;
  ...
  if (use_ublox_receiver_ && nav_pvt_msg_ptr_ != nullptr) {
    orientation = getQuaternionByHeadingFix(nav_pvt_msg_ptr_->heading);
  } else {
...
geometry_msgs::Quaternion GNSSPoser::getQuaternionByHeadingFix(const int heading)
{
  const double yaw = (heading * 1e-5) * M_PI / 180.0;
